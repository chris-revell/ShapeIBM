...
int main() {
    ...
    float hb = 2*len*M_PI/(Nb-1);         // Typical pacing between boundary elements
    ...
    cube fvg1     = cube(Ng,Ng,2,fill::zeros);
    ofstream file1;
    vector<cell> Cells;                     // Vector of cell objects

    // Add a new cell object to the Cells vector until the vector contains Nc components.
    for (int ii=0;ii<Nc;ii++) {
        Cells.push_back(cell(Nb, len, 2*static_cast<float>(ii)*len, 0));
    }
    ...
    file1.open ("boundarypositions.txt", std::ofstream::out | std::ofstream::app);
    // Write initial data to file //
    cout << Cells[0].xb.n_rows << endl;
    cout << Cells[1].xb.n_rows << endl;
    for(int row = 0 ; row < Nb ; row++){
        file1 << Cells[0].xb(0,row) << ", ";
        file1 << Cells[0].xb(1,row) << endl;
        file1 << Cells[1].xb(0,row) << ", ";
        file1 << Cells[1].xb(1,row) << endl;
    }
    //file1 << "" << endl;
    file1.flush();
    ...
    for (int loop_num=0; loop_num<NumLoop; loop_num++) {
        //-- boundary forces --//
        AdjacentForces(fadj,Cells[0].xb,Nb,hb,Spr);            // adjacent
        SecondaryForces(fsec,Cells[0].xb,Nb,hb,Spr,connect);   // secondary
        CenterForces(fcen,Cells[0].xb,Nb,cen,len,Spr,connect); // center
        OppositeForces(fopp,Cells[0].xb,Nb,len,Spr,connect);   // opposite
        for (int ii=0; ii<Nb; ii++){
        ...
        //-- grid forces --//
        BoundToGrid2(fg,Cells[0].xb,fb,Nb,Ng,hg,hg,0.5*hg,xmin,xmax);
        //-- compute grid velocity from NavierStokes --//
        NavierStokes(vg,ug,fg,sg,Ng,rho,mu,dt,hg);
        ug = vg;
        //-- boundary velocities --//
        GridToBound(ub,Cells[0].xb,Nb,vg,Ng,hg,hg,xmin,xmax);
        //-- new position of boundary points --//
        Cells[0].xb = Cells[0].xb + dt*ub;
