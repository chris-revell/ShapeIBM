...
program basicIBM
  ...
  integer xmin=-1
  integer xmax=1
  integer cen=(xmax+xmin)/2      ! fluid domain (square)
  real*8 hg=(xmax-xmin)/Ng    ! fluid mesh width
  real*8 Src=1                ! source strength
  real*8 Spr=100              ! spring stiffness
  real*8 rho=1                ! fluid density
  real*8 mu=1                 ! fluid viscosity
  real*8 dt=0.05              ! time step
  real*8 len=0.2              ! body radius/half length
  integer connect=3              ! spring connections:
                              ! 0-adjacent only, 1-adjacent and secondary
                              ! 2-adjacent and center, 3-adjcent and opposite
  real*8 scaleF=1.0           ! scaling parameter for forces
  real*8 scaleV=1.0           ! scaling parameter for velocities
  integer NumLoop=40             ! number of steps
  integer mod_num=5              ! frequency
  integer Nbs=2
  real*8,dimension(2,2) :: sb
  real*8,dimension(2,2) :: xb = {0}      ! Position of all boundary elements
  real*8 ub[2][Nb] = {0}      ! Velocity of all boundary elements
  real*8 hb                   ! Spacing between boundary elements?
  real*8 fb[2][Nb] = {0}      ! Boundary forces
  real*8 fadj[2][Nb]={0}      ! Adjacent forces array
  real*8 fsec[2][Nb]={0}      ! Secondary forces array
  real*8 fcen[2][Nb]={0}      ! Centre forces array
  real*8 fopp[2][Nb]={0}      ! Opposite forces array
  real*8 xg[Ng][Ng] ={0}      ! Fluid grid array
  real*8 sg[Ng+1][Ng+1]={0}! Fluid grid
  real*8 fg[Ng+1][Ng+1][2]={0}! Grid forces
  real*8 vg[Ng+1][Ng+1][2]={0}! Grid velocities
  real*8 ug[Ng+1][Ng+1][2]={0}! Previous grid velocities
  real*8 sbb[2][2]  ={0}      !
...
contains
  ...
  function CellGrowth
    ...
    disp('  ') disp('  ')
    disp('   This program will run five times:')
    disp('   First  -- you see a growing cell')
    disp('   Second -- you see the cell boundary forces')
    disp('   Third  -- you see the forces that the growing cell exerts on the fluid')
    disp('   Fourth -- you see the fluid velocities')
    disp('   Fifth  -- you see the velocities at the cell boundaries')
    disp('  ')
    disp(' Two different cells are available: ')
    disp(' 1:  round  cell with adjacent and opposite forces')
    disp(' 2:  square cell with adjacent and center forces  ')
    example=input('choose one option:  ')
    ...
    !----------------------!
    !-- development loop --!
    for repeat_num=1:5   ! 5 runs: cells shape forces velocities
