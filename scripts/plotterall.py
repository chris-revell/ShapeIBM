...
grid0 = np.genfromtxt(argv[5]+"/gridpositions0.txt")
grid1 = np.genfromtxt(argv[5]+"/gridpositions1.txt")
nbounddata = np.genfromtxt(argv[5]+"/nbounds.txt")
#data0 = np.genfromtxt(argv[7]+"/fluidvelocities0.txt",skip_header=(Numg*nplot),max_rows=Numg)
#data1 = np.genfromtxt(argv[7]+"/fluidvelocities1.txt",skip_header=(Numg*nplot),max_rows=Numg)
data = np.genfromtxt(argv[5]+"/boundarypositions.txt",delimiter=", ")
...
for nplot,floatnbounds in enumerate(nbounddata):
    ...
    fig.savefig(argv[5]+"/plot{:04d}.png".format(nplot),bbox_inches='tight',padding_inches=0,dpi=200)
    plt.close()

os.system("convert "+argv[5]"/plot{:04d}.png -shave 268x143 "+argv[5]"/plot{:04d}.png")
